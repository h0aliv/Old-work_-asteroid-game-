...
public class Asteroid extends Participant implements ShipDestroyer
{
    ...
    /**
     * When an Asteroid collides with an AsteroidDestroyer or Neutral Destroyer, it expires.
     */
    @Override
    public void collidedWith (Participant p)
    {
        if (p instanceof AsteroidDestroyer)
        {
            ...
            if(size == 2) {
                controller.sound.playbanglarge();
                controller.addParticipant(new Asteroid(rand.nextInt(3), 1, getX(), getY(), rand.nextInt(8) + 1, controller));
                controller.addParticipant(new Asteroid(rand.nextInt(3), 1, getX(), getY(), rand.nextInt(8) + 1, controller));
                controller.score += 20;
            }
            else if(size == 1) {
                controller.sound.playbangmedium();
                controller.addParticipant(new Asteroid(rand.nextInt(3), 0, getX(), getY(), rand.nextInt(8) + 1, controller));
                controller.addParticipant(new Asteroid(rand.nextInt(3), 0, getX(), getY(), rand.nextInt(8) + 1, controller));
                controller.score += 50;
            }
            else {
                controller.sound.playbangsmall();
        ...
        else if(p instanceof NeutralDestroyer) {
            // Expire the asteroid
            Participant.expire(this);
            Random rand = new Random();
            if(size == 2) {
